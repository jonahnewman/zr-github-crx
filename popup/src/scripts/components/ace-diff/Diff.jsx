...
class Diff extends Component {
    ...
    query('getContents',
      ...
      .then((response) => {
        var scrubbed = response.text.replace(/"/g, '\\"')
          .replace(/\n/g, '\\n')
          .replace(/'/g, "\\'")
        window.chrome.tabs.executeScript({code: ContentScript +
  ...
  stopDiff () {
    window.chrome.tabs.query({active: true, currentWindow: true}, function (tabs) {
      window.chrome.tabs.sendMessage(tabs[0].id, {to: 'diff', msg: 'stopDiff'}, function (response) {
      })
    })
    ...
    return (
      <div>
        {this.props.diffing
          ...
          : <div>
            <input type='button' value='diff' onClick={() => { this.setState({dialogOpen: true}) }} />
